...
# USAGE
# python avnet_face_detection.py [--input 0] [--detthreshold 0.55] [--classthreshold 0.35]
...
def run_test(model_name,class_names,annotation_file,boxthreshold=0.3,classthreshold=0.2,save_result=False):
    ...
    densebox_xmodel =model_name #"../../coco/yolov4-coco-xilinx_kv260.xmodel"  #"../../2_5/yolov4-mod-xilinx_kv260.xmodel"
    ...
    dpu_face_detector = FaceDetect(densebox_dpu,class_names,anchors,boxthreshold,classthreshold)
...
def run_camera(model_name,class_names,inputId,boxthreshold=0.3,classthreshold=0.2,save_result=False):
    # Initialize Vitis-AI/DPU based face detector
    #densebox_xmodel = "/usr/share/vitis_ai_library/models/densebox_640_360/densebox_640_360.xmodel"
...
if __name__=='__main__':
    ...
    ap.add_argument("-d", "--boxthreshold", required=False,default=0.55,
        help = "detector threshold (default = 0.55)")
    ap.add_argument("-n", "--classthreshold", required=False,default=0.35,
        help = " threshold clases (default = 0.35)")
    ap.add_argument("-m", "--model_path", default='../../app/2_5/yolov4-mod-xilinx_kv260.xmodel',
        help = "Nombre de modelos")
    
    ap.add_argument("-c", "--class_names", default='coco',
        help = "nombres sobre coco o mi data set")
    
    ap.add_argument("-f", "--annotation_file", default=None,
        help = "path annotations")
    ...
    boxthreshold = float(args["boxthreshold"])
    print('[INFO] face detector - softmax threshold = ',boxthreshold)
    
    classthreshold = float(args["classthreshold"])
    print('[INFO] face detector - NMS threshold = ',classthreshold)

    if args["classthreshold"]=='coco':
        class_names=[
        'person', 'bicycle', 'car', 'motorcycle', 'airplane', 'bus',
        'train', 'truck', 'boat', 'traffic light', 'fire hydrant', 'stop sign',
        'parking meter', 'bench', 'bird', 'cat', 'dog', 'horse', 'sheep', 'cow',
        ...
        class_names =["bm","br","bn","bmt","brt","bnt"]


    if args["classthreshold"]:
        
        run_test(args["model_path"],class_names,args["classthreshold"],boxthreshold,classthreshold,args["save_result"])

    else:
        
        run_camera(args["model_path"],class_names,inputId,boxthreshold,classthreshold,args["save_result"])
