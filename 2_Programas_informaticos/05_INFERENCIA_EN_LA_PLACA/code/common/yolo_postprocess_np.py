...
def yolo_correct_boxes(predictions, img_shape, model_input_shape, is_letterbox=False):
...
def yolo_handle_predictions(predictions, image_shape, num_classes, max_boxes=300, confidence=0.1, iou_threshold=0.4, use_cluster_nms=False, use_wbf=False):
    boxes = predictions[:, :, :4]
...
def yolov4_handle_predictions(predictions, image_shape, num_classes, max_boxes=300, confidence=0.1, iou_threshold=0.4, use_cluster_nms=False, use_wbf=False):
    boxes = predictions[:, :, :4]
    box_confidences = np.expand_dims(predictions[:, :, 4], -1)
    box_class_probs = predictions[:, :, 5:]

    pos = box_confidences[:, :, 0] > confidence
    boxes = boxes[pos]
    ...
    if num_classes == 1:
        ...
        box_scores = box_confidences * box_class_probs
    box_scores[box_scores < confidence] = 0
    ...
    # dets = sorted(dets, key=lambda x: x["box_confidence"], reverse=True)
    # dets = dets[:max_boxes]
    
    for cls_idx in range(num_classes):
        dets = sorted(dets, key=lambda x: x["score"][cls_idx], reverse=True)
        # boxes = np.concatenate([dets[i]["bbox"][None] for i in range(len(dets))], axis=0)
        # iou = box_iou_matrix(boxes, boxes)
        for i in range(len(dets)):
            if dets[i]["score"][cls_idx] == 0:
                continue
            a = dets[i]["bbox"]
            iou = box_diou(np.concatenate([a[None]] + [dets[j]["bbox"][None] for j in range(i + 1, len(dets))], axis=0))
            for j in range(i + 1, len(dets)):
                # b = dets[j]["bbox"]
                # if iou[i, j] > iou_threshold:
                    # dets[j]["score"][cls_idx] = 0
                if iou[j - i - 1] > iou_threshold:
                    dets[j]["score"][cls_idx] = 0
                # if box_iou(np.concatenate([a[None], b[None]], axis=0))[0] > iou_threshold:
                    # dets[j]["score"][cls_idx] = 0
    ...
    for i in range(len(dets)):
        ...
        for cls_idx in range(num_classes):
            prob = scores[cls_idx]
            if prob > 0:
                n_boxes.append(box.reshape(1, 4))
                n_classes.append(cls_idx)
                n_scores.append(prob)
    if len(n_boxes):
        n_boxes = [np.concatenate(n_boxes, axis=0)]
