...
from common.utils_me import get_dataset, get_classes, get_anchors, get_colors, draw_boxes, get_custom_objects
...
def get_prediction_class_records(dpu_detector, annotation_records, anchors, class_names, model_input_shape, conf_threshold, elim_grid_sense, v5_decode, save_result):
    '''
    ...
    for (image_name, gt_records) in annotation_records.items():
        ...
        pred_boxes, pred_classes, pred_scores =  dpu_detector.process(image)
...
def eval_AP(dpu_detector, annotation_lines, anchors, class_names, model_input_shape, eval_type, iou_threshold, conf_threshold, elim_grid_sense, v5_decode, save_result, class_filter=None):
    '''
    ...
    pred_classes_records = get_prediction_class_records(dpu_detector, annotation_records, anchors, class_names, model_input_shape, conf_threshold, elim_grid_sense, v5_decode, save_result)
    AP = 0.0
    print(eval_type)
    if eval_type == 'VOC':
        print('c')
...
def load_eval_model(model_path,detThreshold,nmsThreshold):

    densebox_graph = xir.Graph.deserialize(model_path)
    ...
    class_names =COCO_INSTANCE_CATEGORY_NAMES#["bm","br","bn","bmt","brt","bnt",]
...
def main():
    ...
    dpu_detector = load_eval_model(args.model_path, args.iou_threshold, args.conf_threshold)
    ...
    #def eval_AP(dpu_detector, annotation_lines, anchors, class_names, model_input_shape, eval_type, iou_threshold, conf_threshold, elim_grid_sense, v5_decode, save_result, class_filter=None):
    eval_AP( dpu_detector=dpu_detector,
