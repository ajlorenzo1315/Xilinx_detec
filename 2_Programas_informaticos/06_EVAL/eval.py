...
def yolo_predict_tflite(interpreter, image, anchors, num_classes, conf_threshold, elim_grid_sense, v5_decode):
    ...
    if len(anchors) == 5:
        ...
        pred_boxes, pred_classes, pred_scores = yolo2_postprocess_np(prediction[0], image_shape, anchors, num_classes, model_input_shape, max_boxes=300, confidence=conf_threshold, elim_grid_sense=elim_grid_sense)
    else:
        if v5_decode:
            pred_boxes, pred_classes, pred_scores = yolo5_postprocess_np(prediction, image_shape, anchors, num_classes, model_input_shape, max_boxes=300, confidence=conf_threshold, elim_grid_sense=True) #enable "elim_grid_sense" by default
        else:
            pred_boxes, pred_classes, pred_scores = yolo3_postprocess_np(prediction, image_shape, anchors, num_classes, model_input_shape, max_boxes=300, confidence=conf_threshold, elim_grid_sense=elim_grid_sense)
...
def yolo_predict_pb(sess, model, image, anchors, num_classes, model_input_shape, conf_threshold, elim_grid_sense, v5_decode):
    ...
    if len(anchors) == 6:
        ...
        output_tensor_names = ['graph/conv2d_109/BiasAdd:0', 'graph/conv2d_101/BiasAdd:0', 'graph/conv2d_93/BiasAdd:0']
    elif len(anchors) == 5:
        # YOLOv2 use 5 anchors and have only 1 prediction
...
def get_prediction_class_records(model, model_format, annotation_records, anchors, class_names, model_input_shape, conf_threshold, elim_grid_sense, v5_decode, save_result):
    ...
    for (image_name, gt_records) in annotation_records.items():
        ...
        # support of TF 1.x frozen pb model
            ...
            pred_boxes, pred_classes, pred_scores = yolo_predict_pb(sess, model, image, anchors, len(class_names), model_input_shape, conf_threshold, elim_grid_sense, v5_decode)
...
def main():
    ...
    parser.add_argument(
        '--quantize', default=False, action="store_true",
        help='quantize model'
    )
